...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN 5 */
	...
	uint8_t data[3] = {0x89, 0x55, 0x22};
  /* Infinite loop */
  for(;;)
  {

	CAN_IC_WRITE_REGISTER(0x2c, 0x00);
	CAN_IC_READ_REGISTER(0x2c, &interrupt);

	sendExtendedCANMessage(1, ID, DLC, data);

	while(!interrupt)
	{
		CAN_IC_READ_REGISTER(0x2c, &interrupt);
	}
	CAN_IC_READ_REGISTER(0x2d, &interrupt);
	CAN_IC_READ_REGISTER(0x30, &interrupt);
